...
trait HelperValidator
{
    ...
    function validatorByRules($rules, $input)
    {
        ...
        foreach ($rules as $rule) {
            ...
            if ($rule === 'number') {
                if (isset($_REQUEST[$input]) && !$this->validateNumber($_REQUEST[$input])) {
                    $errors[] = ['rule' => $rule];
                }
            }
            if ($rule === 'mobile') {
                if (isset($_REQUEST[$input]) && !$this->validateMobile($_REQUEST[$input])) {
                    $errors[] = ['rule' => $rule];
                }
            }
            if ($rule === 'password') {
                if (isset($_REQUEST[$input]) && !$this->validateLenPass($input)) {
                    $errors[] = ['rule' => $rule];
                }
            }
            if ($rule === 'persianChar') {
                if (isset($_REQUEST[$input]) && !$this->validatePersianChars($_REQUEST[$input])) {
                    $errors[] = ['rule' => $rule];
                }
            }
        }
    ...
    function validator(array $fields)
    {
        ...
        foreach ($fields as $key => $field) {
            $rules = explode('|', $field);
            $validatorByRules = $this->validatorByRules($rules, $key);
            if (!empty($validatorByRules)) {
                $errors[$key] = $validatorByRules;
            }
        }
        if (!empty($errors)) {
            $_SESSION['error'][$this->pageName()] = [
                'errors' => $errors,
    ...
    {
        $html_last = null;
        $errors = @$_SESSION['error'][$this->pageName()]['errors'];
        $title_error = @$_SESSION['error'][$this->pageName()]['title'];
        if ($errors) {
            foreach ($errors as $key => $error) {
                $input_label = $this->translate($key);
                $html_first = null;
                foreach ($error as $value) {
                    $rule_label = $this->translate($value['rule'], true);
            ...
            unset($_SESSION['error'][$this->pageName()]);
    ...
    function validatePostalCode($data)
    {
        ...
        $validate_number = $this->sanitiseNumber($data);
    ...
    function validateMobile($data)
    {
        ...
        $data = $this->convertNumberToEnglish($data);
